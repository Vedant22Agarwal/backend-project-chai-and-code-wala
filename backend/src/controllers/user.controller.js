...
const loginUser = asyncHandler(async (req, res) => {
  ...
  const userinfo = await User.findOne({
    // by this we are checking either from username or email
    $or: [{ username }, { email }],
  });
...
const updateUserCoverImage = asyncHandler(async (req, res) => {
  ...
  if (userinfo?.coverImage) {
    const publicId = userinfo.coverImage.split("/").pop().split(".")[0];
...
const getUserChannelProfile = asyncHandler(async (req, res) => {
  // yaha do user h ek toh khud tum and ek dusra jiska channel serach kia h
  ...
  const channel = await User.aggregate([
    ...
    {
      $addFields: {
        // ye apne second user model mein fields add ho gai
        subscriberCount: {
          $size: "$subscriber",
        },
    ...
    {
      $project: {
        // what i have give to the frontedn to show
        fullName: 1,
        username: 1,
        subscriberCount: 1,
...
const getWatchHistory = asyncHandler(async (req, res) => {
  const user = await User.aggregate([
    // here mongoose can't act so directly string should not be given
    {
      $match: {
